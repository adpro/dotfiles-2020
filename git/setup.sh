...
if git --version 2>&1 >/dev/null; then
    ...
    # alias for ignored files
    git config --global alias.i 'ls-files . --ignored --exclude-standard --others'
    # alias for non tracked files
    git config --global alias.nt 'ls-files . --exclude-standard --others'    git config --global core.autocrlf input
    ...
    substep_error "git is not installed, cannot set config."
fi
